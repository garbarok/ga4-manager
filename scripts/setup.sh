...
SCOPES="openid,https://www.googleapis.com/auth/cloud-platform,https://www.googleapis.com/auth/analytics.edit,https://www.googleapis.com/auth/analytics.readonly,https://www.googleapis.com/auth/webmasters,https://www.googleapis.com/auth/userinfo.email"
...
if [[ "$AUTH_PATH" == "A" ]]; then
  ...
  if [[ -f "$ADC_FILE" ]]; then
    ...
    if [[ ${#MISSING[@]} -eq 0 ]]; then
      ok "All required scopes already granted (cloud-platform, analytics.edit, analytics.readonly, webmasters)"
      RELOGIN=0
    else
